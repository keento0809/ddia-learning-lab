...
#### 4.4.1 「続きから再開」カードの状態定義

`resume`(in_progress判定)と `overall.lessonsDone`(done集計)は独立したフィールドであり、
`resume === null` は「in_progressのアイテムが無い」ことしか意味しない。この状態は
以下の2パターンに分岐して表示する:

| 状態 | 条件 | 表示 |
| --- | --- | --- |
| 未着手(真の新規ユーザー) | `overall.lessonsDone === 0` かつ `resume === null` | `dashboard.resume.emptyTitle` / `emptyBody` + カリキュラムへのCTA(`emptyCta`) |
| 完了済みだが読みかけ無し | `overall.lessonsDone > 0` かつ `resume === null` | `dashboard.resume.continueTitle` / `continueBody` + 次のレッスンへのCTA(`continueCta`) |
| 再開可能 | `resume !== null` | 直近in_progressアイテムのタイトル・モジュール名 + 再開CTA(`cta`) |
