...
14. 規則13の `git remote -v` はoriginの設定を確認できるだけで、その
    セッションが実際にpushを許可されているか(セッションの認可済み
    リポジトリ範囲・トークン権限等)は別レイヤーであり判定できない。
    origin設定が正しく見えても、実際のpushがセッション側の認可制限で
    拒否される事故が発生している(詳細: STATUS.md 2026-09-12)。そのため
    規則13の確認に加え、着手前に軽量な認可確認として
    `git push --dry-run origin <検証対象ブランチ名>` を実行し、権限エラー
    (403等)が返る場合は実装に着手せずその旨を直ちに報告して停止する。
